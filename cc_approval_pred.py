...
SERVICE_ACCOUNT_FILE = r'C:\Users\dutta\Credit-card-approval-prediction-classification\client_secrets.json'
...
def make_prediction():
    """Function to interact with Google Drive and make a prediction."""
    drive_service = authenticate_google_drive()

    # Example: Upload a file to Google Drive
    file_metadata = {'name': 'example_file.txt'}
    media = MediaFileUpload('example_file.txt', mimetype='text/plain')
    file = drive_service.files().create(body=file_metadata, media_body=media, fields='id').execute()
    st.write(f"File uploaded with ID: {file.get('id')}")
